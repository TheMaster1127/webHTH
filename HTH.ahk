...
if !(SubStr(StrLower(str), 1, 19) = StrLower("Gui, Add, rectangle")) or (SubStr(StrLower(out), 1, 16) = StrLower("Gui, Add, Circle"))
{
        ...
        switch (command) {
          case "play":
            if (typeof parameter === "string") {
              ...
              // Determine the audio MIME type based on the file format
              let mimeType = "audio/mpeg"; // Default MIME type (e.g., MP3)

              // Check for different audio formats
              if (parameter.startsWith("data:audio/wav;base64,")) {
                mimeType = "audio/wav";
              } else if (parameter.startsWith("data:audio/mp3;base64,")) {
                mimeType = "audio/mpeg";
              } else if (parameter.startsWith("data:audio/ogg;base64,")) {
                mimeType = "audio/ogg";
              } else if (parameter.startsWith("data:audio/aac;base64,")) {
                mimeType = "audio/aac";
              } else if (parameter.startsWith("data:audio/m4a;base64,")) {
                mimeType = "audio/mp4";
              } else if (parameter.startsWith("data:audio/flac;base64,")) {
                mimeType = "audio/flac";
              } else if (parameter.startsWith("data:audio/x-aiff;base64,")) {
                mimeType = "audio/x-aiff";
              }
              // Add more conditions for other audio formats...

              // Create a Blob from the bytes with the determined MIME type
          ...
          case "resume":
            ...
            break;
          case "mute":
            audio.volume = 0; // Mute (volume set to 0)
            break;
          case "unmute":
            audio.volume = 1; // Unmute (volume set to 100`%)
            break;
          case "setVolume":
            if (typeof parameter === "number" && parameter >= 0 && parameter <= 100) {
              // Map volume from 0-100 to 0-1
              let volumeLevel = parameter / 100;
              audio.volume = volumeLevel;
            } else {
              console.error("Invalid volume value. Volume must be a number between 0 and 100");
            }
            break;
          default:
